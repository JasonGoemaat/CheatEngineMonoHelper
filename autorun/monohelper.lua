-- TODO: Fix search, sometimes it doesn't find all the values, for example search
-- TODO: Rename formSearch as 'Mono Search' (currently says only 'Mono')
-- TODO: Rename formClass as 'Mono Class' (maybe with class name), currently says only 'Class'
...
function mono.formSelectImage:show()
    -- TODO: Auto-select 'Assembly-CSharp' if present ON FIRST RUN
    -- TODO: Some check to make sure assembly is valid, i.e. after re-enabling, restarting game, etc, then auto-reselect?
    ...
    local foundIndex = 0 -- for 'Assembly-CSharp'
    for i,name in ipairs(self.imageNames) do
      items.add(name)
      if (name == "Assembly-CSharp") then foundIndex = i end
    end
